...
// which iTerm2 uses for its transfer progress display.
export function encodeIterm2(png: Uint8Array): string {
  const b64 = toBase64(png);
  const args = `inline=1;size=${png.length};width=auto;height=auto;preserveAspectRatio=1`;
...
export function encodeKitty(png: Uint8Array, chunkSize = 4096): string {
  const b64 = toBase64(png);
  if (b64.length === 0) {
  ...
  for (let idx = 0; idx < chunks.length; idx++) {
    ...
    const control = idx === 0 ? `a=T,f=100,m=${more}` : `m=${more}`;
    out += `${APC}G${control};${chunks[idx]}${ST}`;
  }
  return out;
}

export function encodeForProtocol(protocol: "iterm2" | "kitty", png: Uint8Array): string {
  return protocol === "kitty" ? encodeKitty(png) : encodeIterm2(png);
}
